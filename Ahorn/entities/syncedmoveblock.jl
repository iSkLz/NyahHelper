module NyahHelperSyncedMoveBlock

using ..Ahorn, Maple
@mapdef Entity "nyahhelper/syncedmoveblock" SyncedMoveBlock(x::Integer, y::Integer, width::Integer=16, height::Integer=16, direction::String="Up", canSteer::Bool=false, colorHue::Integer=122, customSpeed::Real=60.0)
...
for direction in directions, steerable in false:true
    key = "Move Block ($(uppercasefirst(direction))$(steerable ? ", Steerable" :    "")) (Nyah Helper)"
...
Ahorn.editingOptions(entity::SyncedMoveBlock) = Dict{String, Any}(
    "direction" => Maple.move_block_directions
)
Ahorn.minimumSize(entity::SyncedMoveBlock) = 16, 16
Ahorn.resizable(entity::SyncedMoveBlock) = true, true

Ahorn.selection(entity::SyncedMoveBlock) = Ahorn.getEntityRectangle(entity)
...
function Ahorn.render(ctx::Ahorn.Cairo.CairoContext, entity::SyncedMoveBlock, room::Maple.Room)
    x = Int(get(entity.data, "x", 0))
    y = Int(get(entity.data, "y", 0))

    _h = get(entity.data, "colorHue", 122);
    _s = 0.73
    _v = 0.70
    _n1 = _s * _v
    _n2 = _n1 * (1.0 - abs(_h / 60.0 % 2.0 - 1.0))
    _n3 = _v - _n1
    
    if _h < 60
		newColor = ((_n3 + _n1), _n3 + _n2, _n3, 1.0)
    elseif _h < 120
		newColor = (_n3 + _n2, _n3 + _n1, _n3, 1.0)
    elseif _h < 180
		newColor = (_n3, _n3 + _n1, _n3 + _n2, 1.0)
    elseif _h < 240
		newColor = (_n3, _n3 + _n2, _n3 + _n1, 1.0)
    elseif _h < 300
		newColor = (_n3 + _n2, _n3, _n3 + _n1, 1.0)
    else
